...
export class IdentiHeart {
  ...
  constructor(
    ...
    if (typeof canvas === "undefined") {
      throw new Error(
        "Cannot instantiate an IdentiHeart without a target canvas."
      );
    }
  ...
  setCanvas(canvas: HTMLCanvasElement) {
    if (!isDOMElement(canvas)) {
      throw new Error(
        "The parameter for the function IdentiHeart.setCanvas() must be a DOM Element."
      );
    }

    if (canvas.tagName !== "CANVAS") {
      throw new Error(
        "The parameter for the function IdentiHeart.setCanvas() must be a <canvas> element."
      );
    }
  ...
  private drawOutline() {
    ...
    this.context.lineTo(
      this.margin * this.scale,
      this.canvas.height - this.margin * this.scale
    );
    this.context.lineTo(
      this.canvas.width - this.margin * this.scale,
      this.canvas.height - this.margin * this.scale
    );
    this.context.lineTo(
      this.canvas.width - this.margin * this.scale,
    ...
    this.context.lineWidth =
      this.scale * (this.strokeWeight / this.canvas.width);
  ...
  private makePath() {
    ...
    switch (mod) {
      ...
      default:
        ...
        ctx.lineTo(
          this.pos.x + this.cellSize / 2,
          this.pos.y - this.cellSize / 2
        );
